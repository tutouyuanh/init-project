import { KIND, RELICS, applyRelic, buildRun, defaultMods, mulberry } from './Data';
import {
    Actor,
    ActorKind,
    GROUND_Y,
...
export class Sim {
    ...
    spawn(kind: ActorKind, x: number, facing: 1 | -1): Actor {
        const k = KIND[kind];
    ...
    loadRoom(): void {
        ...
        r.enemies.forEach((kind, i) => {
            const e = this.spawn(kind, xs[i] ?? 340 + i * 40, -1);
            e.aiCd = 0.6 + i * 0.3;
        });
        if (r.type === 'relic') this.offerRelics();
        if (r.type === 'rest') this.note('Sit by the lantern. Blood stills.');
        if (r.type === 'start') this.note('Walk east. J saw · K gun · L form · Space step.');
    ...
    pressed(key: keyof InputState): boolean {
        return this.input[key] && !this.prev[key];
    }

    setState(a: Actor, s: Actor['state'], t = 0): void {
    ...
    private startAttack(h: Actor): void {
    ...
    private advanceAttack(h: Actor, dt: number): void {
        h.stateT += dt;
        const form = h.form;
        const combo = h.combo;
        let wind = 0.08;
        ...
        if (form === 'short') {
            if (combo === 0) {
                wind = 0.07; active = 0.09; rec = 0.18; dmgMul = 1; reach = 32;
            } else if (combo === 1) {
                wind = 0.06; active = 0.1; rec = 0.2; dmgMul = 1.15; reach = 34;
            } else {
                wind = 0.1; active = 0.12; rec = 0.28; dmgMul = 1.45; reach = 36;
            }
        } else {
            if (combo === 0) {
                wind = 0.14; active = 0.12; rec = 0.3; dmgMul = 1.35; reach = 50;
            } else {
                wind = 0.16; active = 0.14; rec = 0.36; dmgMul = 1.7; reach = 54;
            }
        }
    ...
    private startTransform(h: Actor): void {
    ...
    private spawnMelee(h: Actor, dmgMul: number, reach: number): void {
    ...
    private updateEnemy(a: Actor, dt: number): void {
    ...
    private advanceEnemyAttack(a: Actor, dt: number): void {
        a.stateT += dt;
        const t = a.stateT;
    ...
    private applyHit(a: Actor, hb: Hitbox): void {
        if (a.kind === 'hunter') {
            if (a.invuln > 0) return;
            this.damageHunter(hb.dmg, hb.x + hb.w / 2 > a.x ? 1 : -1, hb.knock);
            return;
        }
        // parry: gun vs enemy in windup / early attack
    ...
    private startVisceralOn(a: Actor): void {
        const h = this.hunter;
        h.x = a.x - h.facing * 16;
    ...
    private hurtEnemy(a: Actor, dmg: number, knock: number, heavy: boolean): void {
    ...
    private stagger(a: Actor, dur: number): void {
        a.staggerT = dur;
    ...
    private damageHunter(raw: number, fromDir: number, knock: number): void {
        const h = this.hunter;
    ...
    private rallyHeal(amount: number): void {
        const h = this.hunter;
    ...
    private syncHunterAnim(h: Actor): void {
    ...
    animFrame(a: Actor, count: number, fps: number, loop: boolean): number {
